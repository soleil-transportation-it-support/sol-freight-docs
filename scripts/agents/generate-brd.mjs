...
function inferDataSource(field) {
...
function renderMarkdown(flow, fields, sections, requiredSelectors, screenshots) {
  ...
  const requiredFields = fields.filter((f) => reqNameMap.has(f.name) || f.required);
  ...
  const fieldRows = fields
    .map(
      (f) =>
        `| ${f.label} | ${f.tag} | \`${f.name}\` | ${f.type} | ${f.required ? "**Yes**" : "No"} | ${inferMeaning(f, flow.id)} | ${inferValueSpec(f, flow.id)} | ${inferDataSource(f)} | ${f.readOnly ? "Read-only" : "-"} |`
    )
    .join("\n");
