...
|---|---|---|---|---|---|---|---|---|
| MB/L No.* | input | `mbl_no` | text | No | The carrier's official shipment reference number as it appears on the Master Bill of Lading. Must match the booking confirmation issued by the shipping line. | Free text | Entered directly by the user and stored in the shipment record. | - |
| Office* | input | `office` | text | No | The Soleil branch office responsible for filing and managing this shipment. | Free text | Entered directly by the user and stored in the shipment record. | - |
...
| Carrier Bkg. No. | input | `carrier_bkg_no` | text | No | The booking confirmation number provided by the carrier (shipping line). Confirms that space has been reserved on the vessel. | Free text | Entered directly by the user and stored in the shipment record. | - |
| Carrier | input | `carrier` | text | No | Name of the shipping line (carrier) transporting the cargo. | Free text | Entered directly by the user and stored in the shipment record. | - |
| B/L Acct. Carrier | input | `bl_acct_carrier` | text | No | The carrier name used strictly for accounting and invoicing. May differ from the operating carrier in interline or transshipment arrangements. | Free text | Entered directly by the user and stored in the shipment record. | - |
| Shipping Agent | input | `shipping_agent_id` | text | No | The shipping agent responsible for vessel coordination and cargo handling at the port of loading. | Free text | Trade Partner lookup — internal partner database (GET /trade-partners/). Search by company name or partner code. New partners must be created in the Trade Partners module first. | - |
| (Oversea Agent) Consignee | input | `overseas_agent_id` | text | No | The overseas partner or destination consignee who will handle the shipment at the destination. | Free text | Trade Partner lookup — internal partner database (GET /trade-partners/). Search by company name or partner code. New partners must be created in the Trade Partners module first. | - |
...
| Carrier Contract No. | input | `carrier_contract_no` | text | No | Reference number for the service contract or negotiated rate agreement with the carrier. | Free text | Entered directly by the user and stored in the shipment record. | - |
...
| ETD | input | `etd` | date | No | Estimated departure date of the vessel from the port of loading. | Date (YYYY-MM-DD) | Entered directly by the user and stored in the shipment record. | - |
...
| ATD | input | `atd` | date | No | Actual date and time the vessel departed the port of loading. Updated once confirmed. | Date (YYYY-MM-DD) | Entered directly by the user and stored in the shipment record. | - |
| ATA | input | `ata` | date | No | Actual date and time the vessel arrived at the port of discharge. Updated once confirmed. | Date (YYYY-MM-DD) | Entered directly by the user and stored in the shipment record. | - |
| Freight | select | `freight_type` | - | No | Indicates who pays the ocean freight charges — the shipper (Prepaid) or the consignee (Collect). | PREPAID, COLLECT | Fixed dropdown list configured in the application. | - |
| Ship Mode | select | `ship_mode` | - | No | The type of cargo arrangement for this shipment — how the container or cargo space is booked. | FCL, LCL, FAK, BULK | Fixed dropdown list configured in the application. | - |
| SVC Term | select | `svc_term_origin` | - | No | Defines the scope of Soleil's service at the origin — the point from which Soleil takes responsibility for the cargo. | BT, CFS, CY, DOOR, FI, FO, FOT, RAMP, TACKLE | Fixed dropdown list configured in the application. | - |
| SVC Term | select | `svc_term_dest` | - | No | Defines the scope of Soleil's service at the destination — the point until which Soleil is responsible for the cargo. | BT, CFS, CY, DOOR, FI, FO, FOT, RAMP, TACKLE | Fixed dropdown list configured in the application. | - |
...
| OB/L Type | select | `ob_bl_type` | - | No | The format of the original Bill of Lading issued to the shipper — determines how the document is released and negotiated. | EXPRESS BILL OF LADING, ORIGINAL BILL OF LADING, SEAWAY BILL, ELECTRONIC BL (EBL) & IBL | Fixed dropdown list configured in the application. | - |
| Doc Cut-Off Date | input | `doc_cut_off_date` | date | No | The deadline for submitting all shipping instructions (SI) and documents to the carrier. Missing this deadline may delay the shipment. | Date (YYYY-MM-DD) | Entered directly by the user and stored in the shipment record. | - |
| Port Cut-Off Date | input | `port_cut_off_date` | date | No | The deadline for delivering loaded containers to the terminal. Cargo arriving after this time will miss the vessel. | Date (YYYY-MM-DD) | Entered directly by the user and stored in the shipment record. | - |
| VGM Cut-Off Date | input | `vgm_cut_off_date` | date | No | The deadline for submitting the Verified Gross Mass (VGM) — the certified total weight of the packed container — to the carrier, as required by international regulations. | Date (YYYY-MM-DD) | Entered directly by the user and stored in the shipment record. | - |
| Rail Cut-Off Date | input | `rail_cut_off_date` | date | No | For shipments moving by rail to the port, this is the deadline for cargo to be at the rail ramp. | Date (YYYY-MM-DD) | Entered directly by the user and stored in the shipment record. | - |
...
| Container Size | select | `container_size` | - | No | The container equipment type and physical size used for this shipment. | 20GP, 40GP, 40HC, 45HC, 20RF, 40RF | Fixed list — container equipment codes per ISO standards. | - |
...
| Tare Weight (KG) | input | `tare_weight` | text | No | The weight of the empty container itself, as stamped on the container door panel. | Free text | Entered directly by the user and stored in the shipment record. | - |
...
| No. of Packages | input | `number_of_packages` | text | No | Total count of individual packages, boxes, cartons, pallets, or other shipping units. | Free text | Entered directly by the user and stored in the shipment record. | - |
