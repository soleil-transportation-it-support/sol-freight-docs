...
![mbl-edit](../assets/ocean-export/mbl-mbl-edit.png)
_Edit form for a seeded MBL — shows Container Items table with existing rows populated._
...
7. Optional fields may be omitted; the system sends `null` for empty values and stores them as nullable.
...
3. **ETD*** (`name="etd"`) must not be empty; form must block submission and highlight the field with an error message.

## Field Semantics
| Label | Name | Required | Business Meaning | Allowed Values / Format | Data Source / Reference |
|---|---|---|---|---|---|
| MB/L No.* | `mbl_no` | **Yes** | The carrier's official shipment reference number as it appears on the Master Bill of Lading. Must match the booking confirmation issued by the shipping line. | Free text | Entered directly by the user and stored in the shipment record. |
| Office* | `office` | **Yes** | The Soleil branch office responsible for filing and managing this shipment. | Free text | Office lookup (searchable input). Stored as office code on the shipment record. |
...
| B/L Acct. Carrier | `bl_acct_carrier_id` | No | The carrier used for accounting/invoicing. May differ from operating carrier in interline or transshipment scenarios. | Free text | Trade Partner lookup — internal partner database (GET /trade-partners/). Search by company name or partner code. New partners must be created in the Trade Partners module first. |
...
| ETD* | `etd` | **Yes** | Estimated departure date of the vessel from the port of loading. | Date (YYYY-MM-DD) | Entered directly by the user and stored in the shipment record. |
...
| Customer Ref. No. | `direct_customer_ref_no` | No | Customer reference number used only when Direct Master mode is enabled. Cleared automatically when Direct Master is unchecked. | Free text | Entered directly by the user and stored in the shipment record. |
| Customer | `direct_customer_id` | No | Customer party for a Direct Master shipment. | Free text | Trade Partner lookup — internal partner database (GET /trade-partners/). Search by company name or partner code. New partners must be created in the Trade Partners module first. |
| Bill To | `direct_bill_to_id` | No | Billing party for a Direct Master shipment. | Free text | Trade Partner lookup — internal partner database (GET /trade-partners/). Search by company name or partner code. New partners must be created in the Trade Partners module first. |
| Consignee | `direct_consignee_id` | No | Consignee party for a Direct Master shipment. | Free text | Trade Partner lookup — internal partner database (GET /trade-partners/). Search by company name or partner code. New partners must be created in the Trade Partners module first. |
| Sales Type | `direct_sales_type` | No | Sales classification for a Direct Master shipment. | CO-LOAD, FREE CARGO, NOMI | Fixed dropdown list configured in the application. |
| Cargo Type | `direct_cargo_type` | No | Cargo classification for a Direct Master shipment. | AUTOMOBILE (NON-HAZ), BATTERY, GENERAL CARGO, HAZARDOUS, REFRIGERATED, SPECIAL | Fixed dropdown list configured in the application. |
| Sales | `direct_sales` | No | Sales owner or label for a Direct Master shipment. Cleared automatically when Direct Master is unchecked. | Free text | Entered directly by the user and stored in the shipment record. |
...
|---|---|---|---|
| Office | `office` | Lookup — see Data Source | Office lookup (searchable input). Stored as office code on the shipment record. |
...
| B/L Acct. Carrier | `bl_acct_carrier_id` | Lookup — see Data Source | Trade Partner lookup — internal partner database (GET /trade-partners/). Search by company name or partner code. New partners must be created in the Trade Partners module first. |
...
| Customer | `direct_customer_id` | Lookup — see Data Source | Trade Partner lookup — internal partner database (GET /trade-partners/). Search by company name or partner code. New partners must be created in the Trade Partners module first. |
| Bill To | `direct_bill_to_id` | Lookup — see Data Source | Trade Partner lookup — internal partner database (GET /trade-partners/). Search by company name or partner code. New partners must be created in the Trade Partners module first. |
| Consignee | `direct_consignee_id` | Lookup — see Data Source | Trade Partner lookup — internal partner database (GET /trade-partners/). Search by company name or partner code. New partners must be created in the Trade Partners module first. |
| Sales Type | `direct_sales_type` | CO-LOAD, FREE CARGO, NOMI | Fixed dropdown list configured in the application. |
| Cargo Type | `direct_cargo_type` | AUTOMOBILE (NON-HAZ), BATTERY, GENERAL CARGO, HAZARDOUS, REFRIGERATED, SPECIAL | Fixed dropdown list configured in the application. |
